...
ExecHashTableCreate(Hash *node, List *hashOperators)
{
	...
	/*
	 * CSI 3130: Symmetric Hash Join project – **disable multiple batches**.
	 * We force a single in-memory batch and never spill to disk.
	 */
	nbatch = 1;

#ifdef HJDEBUG
	printf("nbatch (forced) = %d, nbuckets = %d\n", nbatch, nbuckets);
#endif
	...
	/*
	 * CSI 3130: since nbatch is always 1, we never want to grow into more
	 * batches.  ExecHashIncreaseNumBatches will see growEnabled == false
	 * and become a no-op.
	 */
	...
	/*
	 * CSI 3130: with nbatch == 1, we never allocate any BufFiles.
	 * innerBatchFile/outerBatchFile stay NULL.
	 */
...
ExecHashIncreaseNumBatches(HashJoinTable hashtable)
{
	...
	/* CSI 3130: if growth disabled, this is a no-op */
						  ...
						  int *batchno)
{
	...
	/*
	 * CSI 3130: **no batching** – all tuples stay in batch 0.
	 */
